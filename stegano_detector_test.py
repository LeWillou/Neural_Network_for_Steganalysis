...
for directory_name in directories[0]:
    for audio_file in audio_files[1]:
        ...
        file_samples_peaks = check_samples.check_peaks_sample('wav_stego/' + directory_name + '/' + audio_file)
        ...
        if (real_result == predicted_results).all():
            print(directory_name +  ' found ! ' + str(counter) + ' found so far..., making it ' + str((counter/320)*100) + '% correct.')
            counter += 1
        else:
